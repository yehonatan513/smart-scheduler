...
export default function PomodoroTimer({ user, subjects, sessions, onUpdate }) {
  ...
  const endSession = useCallback(async (studiedSeconds) => {
    ...
    setTotalStudied(prev => prev + studiedSeconds)
    setConfetti(true)
    playSound()
  ...
  }, [phase, isBreak, workMinutes, breakMinutes])
  ...
  function stopEarly() {
    const elapsed = workMinutes * 60 - secondsLeft
    if (elapsed > 60) endSession(elapsed)
    else { clearInterval(intervalRef.current); setPhase('idle') }
  }

  function getNextSubject() {
    const others = subjects.filter(s => s.id !== selectedSubject?.id && s.remaining > 0)
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            ...
            <button onClick={() => { clearInterval(intervalRef.current); setOpen(false); setPhase('idle') }}
          ...
          {(phase === 'studying' || phase === 'break') && (
            <>
              <div style={{ fontSize: 13, color: isBreak ? 'var(--accent3)' : 'var(--accent)', marginBottom: 16, fontWeight: 600 }}>
                {isBreak ? 'הפסקה' : `לומד: ${selectedSubject?.name}`}
              </div>
          ...
          {phase === 'done' && (
            <>
              ...
              <div style={{ color: 'var(--accent3)', fontSize: 14, marginBottom: 20 }}>
                {savedMsg && `נשמרו ${workMinutes} דקות ל${selectedSubject?.name}`}
              </div>
              ...
              <button className="btn" onClick={() => { setPhase('idle'); setSavedMsg(false) }}
